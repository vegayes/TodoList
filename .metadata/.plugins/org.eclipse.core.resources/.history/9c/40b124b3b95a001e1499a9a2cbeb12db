	...
	public static Connection getConnection() { 
		...
		try {
			...
			if(conn == null || conn.isClosed()) {
				...
				// ★★★★★★★★★★★★★★★★★★Server 이용하면서 새로 추가한 부분★★★★★★★★★★★★★★★★★
				String filePath 
					= JDBCTemplete.class.getResource("/edu/kh/project/sql/driver.xml").getPath();
				
				// file:/C:/server_06/... 
				// getPath를 해주면 file을 빼고 절대 경로로 가져옴. 
				// ==> 파일 객체를 생성할 때 전달된 경로
				
				System.out.println("filePath : " + filePath);
				
				// driver.xml 파일 읽어오기 (-> Server 이용하면서 경로가 바뀌었기 떄문에 변경! ) 
